...
function renderSchool(key){
  ...
  const periodsShort = periods.map(p => p.replace('Semester ', 'S').replace(' (Kreo 04)','').replace(' (Bojongsari 1)','').replace('Kelas ', 'K'));
  ...
  if (s.overallAvg.length >= 2) {
    ...
    sealDelta.textContent = `Data baru ${periods.length} periode`;
  }
...
function drawChart(svg, values, periods, periodsShort, opts){
  ...
  const xScale = i => n > 1 ? padL + i * xStep : padL + innerW / 2;
